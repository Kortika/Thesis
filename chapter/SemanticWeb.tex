...
The semantic web stack enables one to start building a \emph{web of data} using 
...
\gh{I would rephrase as: "...provided that the data is represented in the Resource Description Framework (RDF)[++]". Because at this point the reader doesn't know RDF yet.}
...
data in the linked knowledge graph. Following links is possible due to the nodes in
...
The aforementioned term types are defined in the abstract RDF \sout{syntax}\gh{model} and 
require a concrete \sout{syntax}\gh{serialization} to store and process RDF data. 
Several \sout{syntaxes}\gh{serialization formats} have been developed alongside RDF specifications to represent RDF
knowledge graphs.
...
\subsection{RDF/XML}
The earliest of the serialization formants is RDF/XML\footnote{RDF/XML: \url{https://www.w3.org/TR/rdf-syntax-grammar/}}. 
...
RDF/XML is too verbose and difficult for human to read and understand.
\gh{Don't underestimate humans; maybe replace with "...RDF/XML can be quite verbose and difficult..."}
Moreover, 
it is also not a stream-friendly format for unbounded RDF data generation.
\gh{Perhaps replace with "Moreover, XML is not suited to represent unbounded data streams."}


\subsection{JSON-LD}
JSON-LD\cite{JSON-LD} was developed as 
the modern Linked Data format compared to RDF/XML.
\gh{or to RDF in general (JSON-LD is not an RDF serialisation). See \url{http://manu.sporny.org/2014/json-ld-origins-2/} for a fun read. (don't cite this though :)
We can move this section below the N-Triples section,
and clearly mention that it is possible to represent RDF using JSON-LD, but that it is not intended to.}
