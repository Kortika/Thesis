...
\subsection{RDF-Gen}
RDF-Gen~\cite{rdf_gen} is also based on a SPARQL-like syntax. However, instead of extending SPARQL engines, 
...
keeps the basic graph pattern section of a SPARQL query to reduce the size of the transformation specification. 
...
Data Connectors allow close-to-source processing, 
...
problem of the link discovery problem which is defined as follows:
...
are declarative and have a minimal cognitive burden composing it compared to the
...
verbose than query languages, composing complex mapping documents results in a much more human-readable 
format due to the direct mapping of the attributes.
\gh{Do you have a reference that can support these claims?} 
The following subsections will elaborate more on the related state-of-the-art engines 
which utilize mapping languages in a streaming environment.

\subsection{TripleWave}
Despite the abundance of solutions combining semantic technologies with stream and event processing 
techniques, there was a lack of engines that disseminate and exchange RDF streams on the Web~\cite{triple_wave}. 
TripleWave~\cite{triple_wave} was conceived to play this role; to provide the mechanism to publish and spread RDF streams on the Web. 
It extends R2RML, which only allows ingestion of inputs from relational databases, 
to also consume other formats such as JSON or CSV and other data sources (just like RML, a superset of R2RML). 

TripleWave generates an RDF stream in JSON-LD format, which could be consumed by existing
...
The implementations elaborated on thus far scale insufficiently in terms of velocity and 
