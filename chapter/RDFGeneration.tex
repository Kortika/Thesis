...
\sout{triple}\gh{triples} maps through the use of \textit{rr:parentTriplesMap, rr:joinCondition, rr:child and rr:parent}
properties. Different triples maps might come from separate logical sources, therefore, 
referencing across \sout{these} triples maps \sout{will}\gh{might} require applying the join operator across multiple 
logical sources. 
...
\gh{Change title to "Query based implementations"}
...
with a \sout{traditional} database. Allowing users to query the sources without explicitly 
...
\sout{From what we know}\gh{To the best of our knowledge}, there exist two state-of-the-art query based implementations for transforming 
unbounded non-RDF data to RDF data: SPARQL-Generate and RDF-Gen\sout{, the latter diverging a lot from
the underlying SPARQL syntaxes}.
\gh{I think it doesn't matter how hard they diverge, it's the method that counts.}  
...
orininal data source can be bound.
...
triples. Examples: Optique (\url{https://link.springer.com/chapter/10.1007\%2F978-3-642-41242-4_11}) and Morph-streams (\url{http://dx.doi.org/10.4018/jswis.2012010103})}
...
keeps the \sout{BGP} \gh{basic graph pattern} section of SPARQL query to reduce the size of the transformation specification. 
Thus, it has the most compact mapping template\sout{/query} compared to the other methods mentioned in this paper. 
...
these processes over multiple machines.
\gh{It's rather "Due to parallelization of the tasks..., these processes can be spread over multiple machines."}
...
\emph{Asynchronous Barrier Snapshots}~\cite{flink_fault_tolerance} and \emph{exactly-once} semantic \gh{although these are disabled by default. This reminds me that it would be nice to investigate the influence on the performance, but that's not in the scope of this thesis.}. 
...
support the join operator for executing a binary join, but scalability is not built in the engines to handle multiple streaming 
data sources with high velocity and volume. \sout{There needs to be separate instances started on different 
machines and the coordination mechanism implemented for these engines to scale.}\gh{I would not repeat this.}
An solution
