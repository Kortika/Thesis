...
Moreover, the definitions \gh{or do you mean the *configurations*} of the policies will be based on the 
...
the even-time-based \gh{do you mean "event time based"?} tumbling window $W = \{r_t |  t \in \tau(r_0) \dots \tau(r_n) \}$. 
...
that is immutable throughout the lifetime of a stream processing engine \gh{Maybe replace "engine" with "job". The engine can live longer than the job where the window is defined.}. 
