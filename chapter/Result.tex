...
\gh{Perhaps repeat *how* low (400 msg/s?) because that's specified in the previous chapter...}
 to ensure that 
...
\gh{It's most probably both. I would dare to put it like this: "CPU usage for Dynamic window is approximately 7\% higher than for Tumbling window since it requires extra calculations on the metrics and it increases the throughput causing the RMLStreamer to process more joined records."}  
...
\gh{Figure 8.1: can you add units to the time axes? Then people looking at the charts have an idea how long the evaluation ran.  Figure (e): somewhere make clear in the legenda or at least the caption that the "difference" means "Dynamic - Tumbling". Now it is stated for the red bars only, but it also counts for everything else on that chart.}
...
CPU usage difference of both window mechanisms is similar to that of the workload for latency measurement with relative increase 
...
\gh{Same comments for figures 8.2 and 8.3 as for 8.1}
