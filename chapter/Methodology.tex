...
In this chapter, we introduce a \sout{dynamic approach to windowing}
\gh{isn't it "an approach to dynamic windowing" ?} in data stream processing 
...
\gh{This can be nuanced a bit: what is high? The latency is proportional to the window size. With fixed window this can be higher than necessary... Try to put something like that here.}
...
\gh{What exactly do you mean with "join over aggregation"? Maybe rephrase...} 
...
stream joins as part of the mapping process of non-heterogeneous \gh{why non-heterogeneous (i.e. homogeneous)? One of the strong points of RML is that it tackles heterogeneity...} data streams 
to RDF data streams. 
...
\paragraph{Notations}
\gh{Did you come up with these notations (which are pretty good by the way)? If not, refer to source(s). Same for the notations in Chapter 5.1.}
...
\begin{itemize}
    ...
    \gh{Is this $\epsilon$ defined (formally) somewhere? I don't find it immediately...} 
...
The total cost $m = cost(List_P) + cost(List_C)$ and it is checked against thresholds $\epsilon_l$ and $\epsilon_u$. If  $\epsilon_l \le m \le \epsilon_u$, 
...
\gh{Is this a completely new algorithm, or is it an (adjusted version of) an existing one? In the latter case refer to the original source (and say how it is adjusted if need be)} 
...
\gh{You say it is possible to configure size (and maybe other parameters). If so, can you add an example showing each one in the mapping file(s) in Appendix A? People might be interested in tuning them.} 
